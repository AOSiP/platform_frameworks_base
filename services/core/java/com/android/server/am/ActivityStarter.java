...
class ActivityStarter {
    ...
    private void wrapCameraVersion(Intent intent, String packageName, String callingPackage) {
        try {
            ...
            final boolean hal3Enabled = SystemProperties.get("persist.camera.HAL3.enabled", "0").equals("1");
            final TextUtils.StringSplitter splitter = new TextUtils.SimpleStringSplitter(',');
            final String hal3PackageList = SystemProperties.get("camera.hal3.force.packagelist", "");
                if (hal3PackageList.length() > 0) {
                    splitter.setString(hal3PackageList);
                    for (String str : splitter) {
                        if (str.equals(callingPackage)) {
                            // Google Camera calls Google Photos in preview, otherwise a force close will
                            // occur after the user touches the photo/video preview
                            return;
                        }
                        if (str.contains("/")) {
                            String intentClassName = intent.getComponent().getClassName();
                            String targetClassName = str.replace("/", "");
                            if (!targetClassName.startsWith(intentClassName)) {
