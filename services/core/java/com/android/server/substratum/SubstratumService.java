...
public final class SubstratumService extends SystemService {
    ...
    private final IBinder mService = new ISubstratumService.Stub() {
        ...
        @Override
        public void applyBootanimation(String bytes) {
            if (!isCallerAuthorized(Binder.getCallingUid())) return;
            final long ident = Binder.clearCallingIdentity();
            try {
                if (bytes == null) {
    ...
    private void copyBootAnimation(String inputStream) {
        try {
            clearBootAnimation();
            IOUtils.writeByteArrayToFile(inputStream, IOUtils.SYSTEM_THEME_BOOTANIMATION_PATH);

            IOUtils.setPermissions(new File(IOUtils.SYSTEM_THEME_BOOTANIMATION_PATH),
