...
public class TypographicClock extends TextView implements ColorExtractor.OnColorsChangedListener {
    ...
    private void setWallpaperColors(int mainColor, boolean supportsDarkText) {
        ...
        mPrimaryColor = findColor(mainColor, bgColor, !supportsDarkText, mSystemAccent, mFallbackColor);
        mAmbientColor = findColor(mainColor, Color.BLACK, true, mSystemAccent, mFallbackColor);
        setDarkAmount(mDarkAmount);
    }

    private int findColor(int color, int background, boolean againstDark, int alt, int fallback) {
        if (alt == 0) {
            return color;
        }
        int contrastedColor = NotificationColorUtil.ensureTextContrast(color, background, againstDark);
        ColorUtils.RGBToHSL(Color.red(contrastedColor), Color.green(contrastedColor), Color.blue(contrastedColor), mHslOut);
        if (mHslOut[1] < 0.1f || mHslOut[2] < 0.1f) {
            return findColor(alt, background, againstDark, fallback, 0);
        } else {
