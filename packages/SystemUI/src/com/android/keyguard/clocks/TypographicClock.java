...
public class TypographicClock extends TextView implements ColorExtractor.OnColorsChangedListener {
    ...
    public TypographicClock(Context context, AttributeSet attributeSet, int defStyleAttr) {
        ...
        mFallbackColor = mResources.getColor(R.color.custom_text_clock_top_color, null);
    ...
    public void setClockColors(int primaryColor, int ambientColor) {
        mPrimaryColor = getFallbackColor(primaryColor);
        mAmbientColor = getFallbackColor(ambientColor);
        setDarkAmount(mDarkAmount);
    }

    private int getFallbackColor(int color) {
        ColorUtils.RGBToHSL(Color.red(color), Color.green(color), Color.blue(color), mHslOut);
        if (mHslOut[1] < 0.1f || mHslOut[2] < 0.1f) return mFallbackColor;
        return color;
    }
    ...
    public void onColorsChanged(ColorExtractor extractor, int which) {
        ...
        setWallpaperColors(colors.getMainColor(), colors.supportsDarkText());
    }

    private void setWallpaperColors(int mainColor, boolean supportsDarkText) {
        int scrimColor = supportsDarkText ? Color.WHITE : Color.BLACK;
        int scrimTinted = ColorUtils.setAlphaComponent(ColorUtils.blendARGB(scrimColor, mainColor, 0.5f), 178);
        int bgColor = ColorUtils.compositeColors(scrimTinted, mainColor);
        int normalColor = NotificationColorUtil.ensureTextContrast(mainColor, bgColor, !supportsDarkText);
        int ambientColor = NotificationColorUtil.ensureTextContrast(mainColor, Color.BLACK, true);
        setClockColors(normalColor, ambientColor);
    }
